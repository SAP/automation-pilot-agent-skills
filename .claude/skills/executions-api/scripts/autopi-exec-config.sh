#!/usr/bin/env bash
# SAP Automation Pilot Executions API - Configuration Loader
# Source this file to load config and common functions
#
# Usage: source autopi-exec-config.sh
#
...
# Usage: autopi_api GET "/executions"
#        autopi_api POST "/executions" '{"commandId":"..."}'
autopi_api() {
  local method="$1"
  local endpoint="$2"
  local data="$3"
...
# Usage: autopi_check "/executions/uuid"
