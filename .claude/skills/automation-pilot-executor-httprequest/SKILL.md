...
| **422** | `"Invalid parameters: $(.createResource.output.body \| toObject.error)"` | `status EQUALS 422` |
...
| **≥400** | `"Operation failed: $(.apiCall.output.body \| toObject.error.message)"` | `status GREATER_THAN_OR_EQUAL 400` |

**Error body shapes:** Some APIs return a single error object — use `toObject.error` or `toObject.error.message`. APIs that return an errors array (e.g. CF API v3) use `toObject.errors[0].detail`, where `[0]` selects the first element of the array. Always check the API's actual response shape and pick the matching accessor.

**`progressMessage`:** Always provide a non-null string so operators can see which step is running. Example: `"Calling $(.execution.input.apiUrl)"`.
...
```json
{
  ...
  "errorMessages": [
    ...
    {
      "message": "Invalid parameters: $(.createInstance.output.body | toObject.errors[0].detail)",
      "when": {
        "semantic": "OR",
        "conditions": [
          {
