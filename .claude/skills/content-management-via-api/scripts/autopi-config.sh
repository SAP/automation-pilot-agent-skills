#!/usr/bin/env bash
# SAP Automation Pilot Content API - Configuration Loader
# Source this file to load config and common functions
#
# Usage: source autopi-config.sh
#
...
#   autopi_api_file <method> <endpoint> <file> - Make API call with file data
...
#        autopi_api POST "/commands" '{"name":"test"}'
autopi_api() {
  local method="$1"
  local endpoint="$2"
  local data="$3"
...
# Make an API call with file data
# Usage: autopi_api_file POST "/commands" "command.json"
autopi_api_file() {
  local method="$1"
  local endpoint="$2"
  local file="$3"
...
# Usage: autopi_check "/commands/my-catalog:MyCmd:1"
